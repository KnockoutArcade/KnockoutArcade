...
if calculateFrameData {
	...
	if nonVictim.frameAdvantage && victim.frameAdvantage == false  && victimStun > 0{
	...
	// by 1 each frame.
	else if nonVictim.frameAdvantage == false && victimStun < 1 {
		if (nonVictim.state != eState.IDLE && nonVictim.state != eState.WALKING && nonVictim.state != eState.CROUCHING) frameAdvantage--;
