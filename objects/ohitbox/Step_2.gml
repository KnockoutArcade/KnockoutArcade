...
if (attackProperty[hitboxID].AttackType == eAttackType.GRAB)
{
...
x = owner.x + attackProperty[hitboxID].WidthOffset * sign(owner.image_xscale);
y = owner.y - attackProperty[hitboxID].HeightOffset * sign(owner.image_yscale);
...
if (collisionCheck)
{
	...
	for (var i = 0; i < collisionID; i++;) 
	{
		...
		var gotHitBy = ds_list_find_index(collision_list[| i].owner.hitByGroup, attackProperty[hitboxID].Group)
		if (collision_list[| i].owner != owner && !hasHit && gotHitBy == -1 && !collision_list[| i].owner.invincible) 
		{
			// Throw Teching
			if (attackProperty[hitboxID].AttackType == eAttackType.GRAB && (collision_list[| i].owner.state == eState.GRAB || collision_list[| i].owner.state == eState.HOLD) && collision_list[| i].owner.animTimer <= 8)
			{
			...
			{
			// Grabbing
				if (attackProperty[hitboxID].AttackType == eAttackType.GRAB && collision_list[| i].owner.hitstun < 1 && collision_list[| i].owner.blockstun < 1 && collision_list[| i].owner.grounded == true && collision_list[| i].owner.state != eState.THROW_TECH && collision_list[| i].owner.state != eState.JUMPSQUAT)
				{
					...
					collision_list[| i].owner.animTimer = 0;
					collision_list[| i].owner.x = owner.x + (attackProperty[hitboxID].HoldXOffset * owner.image_xscale);
					...
					// Multiple hitboxes
					hasHit = true;
					ds_list_add(collision_list[| i].owner.hitByGroup, attackProperty[hitboxID].Group);
				...
				else
				{
					...
					if (collision_list[| i].owner.canBlock) && ((attackProperty[hitboxID].AttackType == eAttackType.LOW && collision_list[| i].owner.verticalMoveDir == -1) || attackProperty[hitboxID].AttackType == eAttackType.MID || (attackProperty[hitboxID].AttackType == eAttackType.HIGH && collision_list[| i].owner.verticalMoveDir != -1))
					{
						...
						collision_list[| i].owner.superMeter += floor(attackProperty[hitboxID].MeterGain * 0.5);
						owner.superMeter += floor(attackProperty[hitboxID].MeterGain * 0.75);
					
						collision_list[| i].owner.knockbackVel = attackProperty[hitboxID].KnockBack;
						owner.pushbackVel = attackProperty[hitboxID].Pushback;
						hasHit = true;
						collision_list[| i].owner.blockstun = attackProperty[hitboxID].BlockStun;
			...
			//Hitting	
		else if attackProperty[hitboxID].AttackType != eAttackType.GRAB
		{
			...
			{				
				...
				if (other.counterHitProperty[hitboxID].CounterHitLevel == 1)
				{
				...
				else if (other.counterHitProperty[hitboxID].CounterHitLevel == 2)
				{
				...
				else if (other.counterHitProperty[hitboxID].CounterHitLevel == 3)
				{
			...
			oGameManager.frameAdvantage = 0;
				...
				//Draw hit effect
			var particle = instance_create_layer(x + (attackProperty[hitboxID].ParticleXOffset * owner.image_xscale), y - attackProperty[hitboxID].ParticleYOffset, "Particles", oParticles);
			with (particle) 
			{
				sprite_index = asset_get_index(other.attackProperty[other.hitboxID].ParticleEffect);
				image_xscale = sign(other.owner.image_xscale);
				lifetime = other.attackProperty[other.hitboxID].ParticleDuration;
			}
		}
	}
}