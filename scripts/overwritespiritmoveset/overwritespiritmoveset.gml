...
// Some moves have different versions depending on whether they are performed in Spirit OFF or Spirit ON.
// This script switches those moves to the appropriate versions.
function OverwriteSpiritMoveset(spiritOff)
{
	for (var i = 0; i < array_length(selectedCharacter.MoveData); i++;)
	{
		if (selectedCharacter.MoveData[i].SpiritData.OnlyInSpiritOff == spiritOff)
		{
			var nextMove = {};
