...
function PerformProjectile(playerOwner, spiritOwner)
{
	...
	else if (!hasSpawnedHitboxes)
	{
		...
		for (var i = 0; i < hitboxPropertiesCopy.NumberOfHitboxes; i++;)
		{
			hitbox = instance_create_layer(x + (hitboxPropertiesCopy.AttackProperty[i].AttackWidth * other.image_xscale) + 0.5, y - hitboxPropertiesCopy.AttackProperty[i].HeightOffset, "hitboxes", oHitbox);
			ds_list_add(hitboxID, hitbox);
			with(hitbox)
			{
				hitboxID = i;
